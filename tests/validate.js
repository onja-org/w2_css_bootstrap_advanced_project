...
function validateHTML() {
    ...
    test('Has navbar with proper Bootstrap classes', () => {
    ...
    test('Has navbar brand with logo', () => {
        const brand = document.querySelector('.navbar-brand');
        return brand && (brand.querySelector('img') || brand.textContent.trim());
    }, 'phase1');

    test('Has mobile toggle button with correct attributes', () => {
        const toggle = document.querySelector('.navbar-toggler');
        return toggle &&
               toggle.getAttribute('data-bs-toggle') === 'collapse' &&
               toggle.getAttribute('data-bs-target');
    }, 'phase1');

    test('Has exactly 5 navigation items', () => {
        const navItems = document.querySelectorAll('.navbar-nav .nav-item');
        return navItems.length === 5;
    }, 'phase1');

    test('Has collapsible navbar section', () => {
        const collapse = document.querySelector('.navbar-collapse');
        const toggle = document.querySelector('.navbar-toggler');
        return collapse && collapse.id && toggle &&
               toggle.getAttribute('data-bs-target') === `#${collapse.id}`;
    }, 'phase1');
    ...
    test('Has hero section with proper structure', () => {
        const hero = document.querySelector('.hero-section, section[id="home"], .min-vh-100');
        return hero !== null;
    }, 'phase2');

    test('Has two-column layout in hero', () => {
        const hero = document.querySelector('.hero-section, section[id="home"], .min-vh-100');
    ...
    test('Contains hero headline about AI workflow', () => {
        const headlines = document.querySelectorAll('h1, .display-4, .display-1, .display-2, .display-3');
        return Array.from(headlines).some(h =>
            h.textContent.includes('Revolutionize') && h.textContent.includes('Workflow')
        );
    }, 'phase2');
    ...
    test('Has responsive hero image', () => {
        const hero = document.querySelector('.hero-section, section[id="home"], .min-vh-100');
        if (!hero) return false;
    ...
    test('Has features section heading', () => {
        const headings = document.querySelectorAll('h1, h2, h3, h4, h5, h6');
        return Array.from(headings).some(h =>
            h.textContent.includes('Why Choose') || h.textContent.includes('Features')
        );
    }, 'phase3a');

    test('Has three or more feature cards', () => {
        const cards = document.querySelectorAll('.card');
        return cards.length >= 3;
    }, 'phase3a');

    test('Has equal height cards', () => {
        const cards = document.querySelectorAll('.card.h-100');
        return cards.length >= 3;
    }, 'phase3a');

    test('Contains AI, Integration, and Support features', () => {
    ...
    test('Has team section heading', () => {
        const headings = document.querySelectorAll('h1, h2, h3, h4, h5, h6');
        return Array.from(headings).some(h =>
            h.textContent.includes('Meet Our Team') || h.textContent.includes('Team')
        );
    }, 'phase3b');

    test('Contains all three team members', () => {
        const content = document.body.textContent;
        return content.includes('Sarah Johnson') &&
               content.includes('Mike Chen') &&
               content.includes('Lisa Rodriguez');
    }, 'phase3b');

    test('Has team member images', () => {
        const teamImages = document.querySelectorAll('img[alt*="Sarah"], img[alt*="Mike"], img[alt*="Lisa"]');
        return teamImages.length >= 3;
    }, 'phase3b');

    test('Uses responsive grid for team cards', () => {
    ...
    test('Has pricing section heading', () => {
        const headings = document.querySelectorAll('h1, h2, h3, h4, h5, h6');
        return Array.from(headings).some(h =>
            h.textContent.includes('Choose Your Plan') || h.textContent.includes('Pricing')
        );
    }, 'phase3c');

    test('Contains all pricing plans', () => {
        const content = document.body.textContent;
    ...
    test('Has pricing information', () => {
        const content = document.body.textContent;
        return content.includes('$29') &&
               content.includes('$79') &&
    ...
    test('Footer has dark background', () => {
        const footer = document.querySelector('footer');
        return footer && footer.classList.contains('bg-dark');
    }, 'phase4');

    test('Has multi-column footer layout', () => {
        const footer = document.querySelector('footer');
    ...
    test('Contains contact information', () => {
        ...
        return content.includes('TechFlow') && content.includes('2024');
    }, 'phase4');

    test('Has social media icons', () => {
    ...
    test('Has Bootstrap CSS linked', () => {
        const bootstrapLink = document.querySelector('link[href*="bootstrap"]');
        return bootstrapLink !== null;
    }, 'general');

    test('Has Font Awesome CSS linked', () => {
        const fontAwesome = document.querySelector('link[href*="font-awesome"]');
        return fontAwesome !== null;
    }, 'general');

    test('Has Bootstrap JavaScript linked', () => {
    ...
    Object.entries(validationResults.sections).forEach(([phase, results]) => {
        if (results.total > 0) {
            ...
            const status = percentage === 100 ? 'COMPLETE' :
                          percentage >= 80 ? 'GOOD' :
                          percentage >= 60 ? 'NEEDS WORK' : 'INCOMPLETE';

            const color = percentage === 100 ? 'green' :
                         percentage >= 80 ? 'cyan' :
                         percentage >= 60 ? 'yellow' : 'red';

            console.log(`${phase.toUpperCase().padEnd(10)} ${colorize(status.padEnd(12), color)} ${results.passed}/${results.total} (${percentage}%)`);
        }
    });
    ...
    console.log(colorize('\n' + '='.repeat(50), 'cyan'));
    console.log(`${colorize('OVERALL SCORE:', 'bright')} ${validationResults.passed}/${totalTests} tests passed (${overallPercentage}%)`);
